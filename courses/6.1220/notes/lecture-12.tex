...
\begin{itemize}
    ...
    \item The dual of the dual is itself.
\end{itemize}
...
\begin{remark}
    Given a solution to the primal, we can find a solution to the dual.  We do this by identifying the constraints in the primal that are slacked, ignoring those, and setting everything else to be strict.  This is DCW-Equality.
\end{remark}

We can use LP to solve the MST problem.  Give indicator variables $x_e$ for all edges with $x_e \geq 0$.
\[ \text{Minimize } \sum_{e \in E} w(e) x_e \text{ given } \sum_{e \text{ crosses } \Pi} x_e \geq |\Pi| - 1 \text{ and } x_e \geq 0. \]
...
\[ \text{Maximize } \sum_{\text{partitions } \Pi} (|\Pi| - 1)y_{\Pi} \text{ given } \sum_{e \text{ crosses } \Pi} y_{\Pi} \leq w(e) \text{ and } y_{\Pi} \geq 0. \]
