...
\begin{problem}{Disjoint Set}
    Consider a collection of pairwise disjoint sets $\text{DS} := \{ S_1, S_2, \dots, S_r\}$, where each set $S_i$ contains a single representative $\textsc{Rep}[S_i] \in S_i$.  Suppose $\text{DS}$ supports the following operations:
    \begin{itemize}
        \item $\textsc{Make-Set}(x)$: Adds $\{x\}$ to $\text{DS}$, with $\textsc{Rep}[\{x\}] = x$.
        \item $\textsc{Find-Set}(x)$: Returns $\textsc{Rep}[S(x)]$, where $S(x)$ is the set containing $x$.
...
\begin{solution*}{Disjoint Set \#1}
    ...
    \[ \textsc{Make-Set}: \Theta(1) ~~~~~~ \textsc{Find-Set}(x): \Theta(|S(x)|) ~~~~~~ \textsc{Union}(x, y): \Theta(|S(x)| + |S(y)|). \]
\end{solution*}
...
\begin{proof}
    Any individual element has its pointer updated at most $O(\log n)$ times, so \textsc{Union} steps take at most $O(n \log n)$ time.  Every other step takes $\Theta(1)$ time.  And $k \geq n$.  ~ $\blacksquare$
\end{proof}
...
\begin{itemize}
    ...
    \[ \sum_{i = 1}^k \hat{c_i} = \sum_{i = 1}^k \left [ c_i + (\Phi_i - \Phi_{i - 1}) \right ] = (\Phi_k - \Phi_0) + \sum_{i = 1}^k c_i \geq \sum_{i = 1}^k c_i. \]
...
\begin{solution}{Disjoint Set \#4}
    ...
    \begin{itemize}
        ...
        \item $\textsc{Union}(x, y)$: Find the roots, then point one root to the other.  This is $\Theta(|S(x)| + |S(y)|)$ in the worst case.
    \end{itemize}
\end{solution}
...
\begin{solution}{Disjoint Set \#5}
    ...
    \begin{itemize}
        ...
        \item $\textsc{Union}(x, y)$: Also $O(\log n)$.
    \end{itemize}
\end{solution}
...
\begin{solution}{Disjoint Set \#6}
    ...
    \[ (u_1 \to u_2 \to \dots \to u_m \to R) ~ \implies ~ (u_1 \to R, ~~ u_2 \to R, ~~ \dots, ~~ u_m \to R) \]
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item $\textsc{Find-Set}(x)$: Suppose this step reroots $u_1 \to u_2 \to \dots \to u_m \to R$.  Then:
        \[ \Delta \Phi_i = \sum_{j = 2}^{m} \left [ \log (L(u_j) - L(u_{j - 1})) - \log (L(u_j)) \right ] = \sum_{j = 2}^m \log \left ( 1 - \dfrac{L(u_{j - 1})}{L(u_j)}\right). \]
        We can now bound $\hat{c_i}$, with a computational trick that makes it clean:
        \begin{align*}
            \hat{c_i} = m + \sum_{j = 2}^m \log \left(1 - \dfrac{L(u_{j - 1})}{L(u_j)} \right) = 1 + \sum_{j = 2}^m \left [ 1 + \log \left(1 - \dfrac{L(u_{j - 1})}{L(u_j)}\right) \right ].
        \end{align*}
        Importantly, we have the following (check this!):
        \[ 1 + \log \left(1 - \dfrac{L(u_{j - 1})}{L(u_j)}\right) \begin{cases} \in [0, 1] & \text{for at most } O(\log n) \text{ values of } j. \\ <0 & \text{for all other values of } j. \end{cases} \]
        This justifies why $\textsc{Find-Set}$ operations have amortized cost $O(\log n)$.
        \item $\textsc{Union}(x, y)$: This step is the combination of $\textsc{Find-Set}(x)$ and $\textsc{Find-Set}(y)$, plus a merge.

        The two $\textsc{Find-Set}$ operations have $O(\log n)$ amortized cost.  The merge has a fake cost of:
        \[ \log (L(x) + L(y)) - \log (L(x)) = \log \left ( 1 + \dfrac{L(y)}{L(x)} \right) = O( \log n). \]
        So the $\textsc{Union}$ operation also has amortized cost $O(\log n)$.
    \end{itemize}
    And so this solution also has $O(\log n)$ amortized cost overall. ~ $\blacksquare$
\end{proof}

It turns out that if you \emph{combine} the approaches of Disjoint Set \#5 and Disjoint Set \#6, you get a solution with $O(\alpha(n))$ amortized cost, where $\alpha$ is the \textbf{Inverse-Ackermann Function}.  For most reasonable $n$, we have $\alpha(n) \leq 4$.
