...
\textbf{Definition (Convolution).} Given sequences $\{a_j\}_{j = 0}^m$ and $\{b_k\}_{k = 0}^n$ of numbers, their \textbf{convolution} $a \ast b$ is:
...
\begin{solution}{FFT}
    ...
    \begin{quote}
        \textbf{Fact.} Any degree-$n$ polynomial is uniquely determined by its evaluations on any $n + 1$ distinct values.
    \end{quote}
    A good choice of values to evaluate our polynomials on, it turns out, is the full set of $(2^t)^{\text{th}}$ roots of unity.
    \begin{quote}
        \textbf{Claim (Discrete Fourier Transform).} Let $n$ be a power of two, and let $\omega := e^{\frac{2\pi i}{n}}$.  For any polynomial $p$ with degree $(n - 1)$ or lower, we can compute $\{p(\omega^{k}) \}_{k = 0}^{n - 1}$ in $O(n \log n)$ time.
        ...
        \begin{itemize}
            ...
            \item In $2T\left(\frac{n}{2}\right)$ time, compute $\{p_{\text{even}}(\omega^{2k})\}_{k = 0}^{n / 2 - 1}$ and $\{p_{\text{odd}}(\omega^{2k})\}_{k = 0}^{n / 2 - 1}$.
            ...
            \[ p(\omega^k) = \begin{cases}
                ...
                p_{\text{even}}\left(\omega^{2k - n}\right) - \omega^{k - n/2} \cdot p_{\text{odd}}\left(\omega^{2k - n}\right) & \text{if $k \geq \frac{n}{2}$.}
            \end{cases} \]
        \end{itemize}
        This yields $T(n) = 2T\left(\frac{n}{2}\right) + O(n)$, which solves to $T(n) = O(n \log n)$. ~ $\square$
    \end{quote}
    Thus, we select $\omega = e^{\frac{2\pi i}{N}}$ for some power-of-two $N > m + n$, then compute $\{p(\omega^k) \cdot q(\omega^k)\}_{k = 0}^{N - 1}$ in $O((m + n) \log (m + n))$ time.  It remains to invert the DFT process above to extract $p(x) \cdot q(x)$ from here.
    \begin{quote}
        \textbf{Claim (Inverse Discrete Fourier Transform).} Given the values $\{p(\omega^k)\}_{k = 0}^{n - 1}$ for some polynomial $p$ with degree $(n - 1)$ or lower, we can compute $p(x)$ in $O(n \log n)$ time.
        ...
        Therefore, $\overline{W}W = nI$, so all we need to do is perform the DFT (with $\overline{\omega}$ in place of $\omega$) on the LHS and divide by $n$. ~ $\square$
    \end{quote}
