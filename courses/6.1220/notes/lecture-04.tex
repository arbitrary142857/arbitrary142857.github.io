...
\begin{problem}{Self-Organizing List}
    ...
    An adversary gives a sequence $R = (x_1, x_2, \dots, x_k)$ online.  How should we perform transpositions to minimize cost?
\end{problem}

First, some preliminary observations about the problem:
\begin{itemize}
    \item Swapping $L[i]$ and $L[i + 1]$ decreases the cost of $\textsc{Access}(L[i + 1])$ by $1$ and increases the cost of $\textsc{Access}(L[i])$ by $1$.
    \item Transpositions only help if, say, $L[i + 1]$ is accessed more than once.
...
In contrast, the \emph{online} algorithm called ``don't transpose anything'' is $\Theta(n \cdot |R|)$.  Can we do better---be competitive?
...
\begin{proof}
    ...
    where \textsc{Opt} uses $t_i$ transpositions on the $i^{\text{th}}$ step.  (Here, $c_i$ and $c_i^*$ are the costs of \textsc{Mtf} and \textsc{Opt}, respectively.) Let's also define:
    ...
    \end{itemize}
    Given a state $(L_{i - 1}, L_{i - 1}^*)$ and a query $\textsc{Access}(x_i)$, we put every element $x \neq x_i$ in one of four categories:
    \begin{itemize}
    ...
    \begin{align*}
        \hat{c_i} = c_i+ \Delta \Phi_i \leq \ & 2(|A_i| + |B_i| + 1) - 1 + 2(|A_i| - |B_i| + t_i) \\
        = \ & 4|A_i| + 2t_i + 1 \\
