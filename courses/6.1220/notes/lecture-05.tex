...
\begin{boxed-text}[34em]
    ...
    For example, when $\beta = 0.2$, this upper-bounds the probability \\ that a binomial variable exceeds $20\%$ its expected value.
\end{boxed-text}

\textbf{Claim.} With high probability (e.g., at least $1 - \frac{1}{n}$), Quick Sort runs in $O(n \log n)$ time.
