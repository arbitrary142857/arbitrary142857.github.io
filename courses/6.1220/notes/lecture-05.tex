...
\begin{solution}{Randomized Solution \#1}
    Pick a random nonzero $v \in \mathbb{F}_2^n$.  Check whether $Cv = A(Bv)$ in $\Theta(n^2)$ time.
\end{solution}
...
\begin{solution}{Paranoid Select}
    Randomly try pivots $x \in A$ until $x$ is $\frac{3}{4}$-balanced, that is, $\frac{1}{4}|A| \leq \textsc{Rank}(x) \leq \frac{3}{4}|A|$.
\end{solution}

In expectation, Paranoid Select requires two tries to pick a good $x$, yielding $\Theta(N)$ runtime in expectation.

\begin{remark}
    Though the analysis of Paranoid Select is simpler, it turns out Quick Select is just better in practice.
\end{remark}
...
\begin{solution}{Paranoid Quick Sort}
    Pick a random pivot $x \in A$, and compute $L(x)$ and $G(x)$.  Then write $\textsc{QS}(L) + \{x\} + \textsc{QS}(G)$.  Pick $x$ by randomly trying pivots until a $\frac{3}{4}$-balance is achieved.
\end{solution}
...
\begin{boxed-text}[34em]
    \textbf{Chernoff Bound.} For a binomial random variable $Y = B(n, p)$ and $\beta \in [0, 1]$: \vspace{-0.3cm}
    \[ \mathbb{E}[Y] = np ~~ \text{ and } ~~ \mathbb{P}[Y \geq (1 + \beta) \cdot np] \leq e^{-\frac{1}{3} \beta^2  np}. \] \vspace{-0.8cm}
    
    For example, when $\beta = 0.2$, this upper-bounds the probability \\ that a binomial variable exceeds its expected value by $20\%$.
\end{boxed-text}

\begin{claim}
    With high probability (e.g., at least $1 - \frac{1}{n}$), Quick Sort runs in $O(n \log n)$ time.
\end{claim}
...
\begin{proof}
    ...
    \end{cases} \]
    We want $a_i$ to encounter good pivots, so we want $Y_i = \sum_{k = 1}^L Y_{i, k}$ to (probably) be small.  Well, $Y_i = B(L, 0.5)$, so:
    ...
    \end{enumerate}
    We claim $L = 300 \cdot \ln n$ works.  For part 1, the condition $Y_i \leq 0.6L$ (i.e. at least $0.4L$ good pivots) tells us:
    \[ \text{size of the subset containing } a_i \text{ at depth } L \leq n \cdot \left(\dfrac{3}{4}\right)^{0.4L} = n \cdot n^{\ln \left(\frac{3}{4}\right) \times 0.4 \times 300} \approx n^{-33} < 1. \]
