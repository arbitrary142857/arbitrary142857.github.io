...
\end{itemize}
\begin{remark}
    An easy exercise: find a streaming algorithm for finding the maximum element of a list, such that\dots
    \begin{itemize}
...
\begin{proof}
    ...
    \end{align*}
    Notably, $\delta \approx 1 - (e^{-1/(1 + \epsilon)} - e^{-1/(1 - \epsilon)})$ does not tend to be very small for large $n$---especially for small $\epsilon$. ~ $\blacksquare$
\end{proof}
...
\begin{proof}
    ...
    Otherwise, $\tilde{f}(X) \geq (1 + \epsilon) d$ means that at least $k$ hash values are at most $L = k u^3 \cdot \left [ (1 + \epsilon) d \right ]^{-1}$.  Consider indicator variables $Y_i$ that measure whether the $i^{\text{th}}$ distinct element is hashed to a value at most $L$.  If $Y := \sum_{i = 1}^d Y_i$, then:
...
The space is $O(k \log u) = O\left(\frac{\log u}{\epsilon^2}\right)$, and the update time is $O(\log k)$; the $\log k$ comes from max-heap insertion operations.

\begin{remark}
    We used the term ``$2$-wise independent hashing'' earlier.  Here are the details of what that means:
    \begin{quote}
...
\begin{proof}
    ...
    In this way, instances of $x$ form disjoint pairs with different values $x_i$ later in the stream.  And \emph{removing} any one of these pairs from the stream does not affect the final answer of the algorithm!

    But if we remove all such pairs, the result is an input stream of only $M$'s---the leftovers are all copies of the final $x$, and $M$ is too frequent to be paired away entirely.  So $M$ must be the final answer. ~ $\blacksquare$
...
\begin{solution}{MG82}
    ...
    \begin{itemize}
        ...
        \item Otherwise, decrement all $c$.  Set any $x$ whose $c$ becomes $0$ to \textsc{Null}.
    \end{itemize}
...
\begin{proof}
    ...
    \[ \{(c_1, x_1), (c_2, x_2), \dots, (c_{k - 1}, x_{k - 1})\} \stackrel{x_i}{\longrightarrow} \{(c_1 - 1, x_1), (c_2 - 1, x_2), \dots, (c_{k - 1} - 1, x_{k - 1})\} \]
    can be viewed as an instance of $x_i$ ``eliminating'' an instance of each of $x_1, x_2, \dots, x_{k - 1}$.

    Removing any of these $k$-tuples from the input stream does not affect the final result of the algorithm.  Remove $k$-tuples until you cannot any longer; the resulting stream still has every $x$ that appears more than $\frac{1}{k}$ of the time. ~ $\blacksquare$
\end{proof}
...
\begin{solution}{Count-Min}
    Take $d := \lceil \log_2 \delta^{-1} \rceil$ and $m := \lceil 2 \epsilon^{-1} \rceil$.  Furthermore, consider a $2$-wise independent hash family $\mathcal{H} := \{ h_i : \mathcal{U} \to \{1, 2, \dots, m\}\}$ and sample $d$ hash functions $\{h_1, h_2, \dots, h_d\}$ from $\mathcal{H}$.
    ...
    Based on these frequency estimates, we select the heavy hitters to be all $x_k$ with $\#(x_k) \geq \frac{n}{k}$, where $n := |X|$.  (Finding these $x_k$ requires updating a min-heap that keeps track of high-frequency $x_k$ incrementally.)
\end{solution}

In other words, Count-Min has $d$ hash functions, each using $m$ space, to try to estimate the frequency of each $x_k$.  Hash collisions will cause hash functions to overestimate, so we use the minimum estimate across all $h_i$.
...
\begin{proof}
    ...
    \begin{quote}
        ...
        \emph{Proof:} Count-Min returns such an $x_k$ only if every single $h_i$ overestimates by at least $\epsilon n$.  Well,
        ...
        (This step requires $\mathcal{H}$ to be universal.) Therefore, by Markov's inequality,
        ...
        So the probability that every single $h_i$ overestimates by too much is at most $\left(\frac{1}{2}\right)^{d} \leq \delta$. ~ $\square$
    \end{quote}
    And that's why we chose $d := \lceil \log_2 \delta^{-1} \rceil$ and $m := \lceil 2 \epsilon^{-1} \rceil$. ~ $\blacksquare$
\end{proof}

\textbf{Definition (Jaccard Similarity).} Given sets $S$ and $T$, their \textbf{Jaccard similarity} is $J(S, T) := \frac{|S \cap T|}{|S \cup T|}$.
...
\begin{solution}{Approximate Similarity Search}
    ...
    Then output \textsc{Yes} if there is some set $A_j$ and some $1 \leq \ell \leq L$ such that $\sigma_{\ell}(A_j) = \sigma_{\ell}(A)$, and \textsc{No} otherwise.
    \begin{quote}
        \textbf{Claim.} If $J(A_j, A) = \lambda$, then $\mathrm{Pr}[\sigma_{\ell}(A_j) = \sigma_{\ell}(A) \text{ for some } \ell] \geq 1 - (1 - \lambda^t)^L$. 
    \end{quote}
    Thus, we should tune the parameters $t$ and $L$ so that $1 - (1 - (s')^t)^L$ is small and $1 - (1 - s^t)^L$ is big.  (Bigger $t$ eliminates false positives, whereas bigger $L$ eliminates false negatives.)
\end{solution}
