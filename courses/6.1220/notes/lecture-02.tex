...
\textbf{Definition.} For any $\frac{1}{2} \leq c \leq 1$, we say $x \in A$ is $c$-balanced if $\max \{ |L(x)|, |G(x)|\} \leq cN$.
...
How do we find $c$-balanced element?  The trick is to use Rank Finding again!
...
\begin{proof}
    ...
    \begin{quote}
        ...
        \emph{Proof:} The constant $\frac{7}{10}$ is $1 - \frac{1}{2} \times \frac{3}{5}$.  More generally, when $5 = 2k + 1$, we have $c = \frac{3k + 1}{4k + 2}$.  ~ $\square$
    \end{quote}
...
\begin{solution}{Karatsuba}
    Write $a = y + 2^{N/2} \cdot x$ and $b = w + 2^{N/2} \cdot z$, so $a = [x] [y]$ and $b = [w] [z]$ and:
    \begin{align*}
