...
\begin{claim}
    ...
    \begin{itemize}
        \item If we remove $e$ from $T$, we get two connected components $S$ and $T$ (containing $u$ and $v$, respectively).
        \item For any $e' \in E$ connecting $S$ and $T$, the subgraph $T' \setminus \{ e \} \cup \{e'\}$ is a spanning tree.
    \end{itemize}
\end{claim}
...
\begin{solution}{Kruskal's Algorithm}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item For each $(u_i, v_i)$ such that $\textsc{FindSet}(u_i) \neq \textsc{FindSet}(v_i)$, add $e_i$ to $F$ and apply $\textsc{Union}(u_i, v_i)$.
        \end{itemize}
...
\begin{proof}
    ...
    Suppose $F \subseteq T$ initially, for some MST $T$.  Now we draw an edge $e = (u_i, v_i)$ between $A := \textsc{FindSet}(u_i)$ and $B := V \setminus A$.  (Intuitively, we should think of edge $e$ as bridging across the ``cut'' $V = A \sqcup B$.)
...
There's only three major steps.
    \begin{enumerate}
        \item Initialize the $\textsc{Union-Find}$ data structure takes $O(|V|)$ time.
        \item Sorting $E$ takes $O(|E| \log |E|) = O(|E| \log |V|)$ time (since $|E| = O(|V|^2)$).
        \item All of the $\textsc{Union}$ operations take $O(|E| \cdot \alpha(|V|))$ time.
    \end{enumerate}
...
\begin{remark}
    ...
    \begin{itemize}
        ...
        \item The method $Q.\textsc{DeleteMin}()$ returns the lowest-value key of $Q$ and deletes it in $O(\log n)$ time.
...
\begin{solution}{Prim's Algorithm}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \item Initialize $\textsc{Parent}[u] = \textsc{Null}$ for all $u \in V$.
        \end{itemize}
        ...
        \begin{enumerate}
            ...
            \item Now that we've added $u$ to $F$, we need to make some updates.  For each $v$ neighboring $u$\dots
            \begin{itemize}
...
\begin{proof}
    ...
    It's actually the same argument as before.  Consider the cut $V = S \sqcup (V \setminus S)$.  You can do \emph{whatever} to bridge this cut and the result will still be an ST, minimal if you do the ``\emph{whatever}'' with minimal weight. ~ $\blacksquare$
\end{proof}

This runs in $O(|E| + |V|\log |V|)$ time, since\dots
\begin{itemize}
    \item We have $|V|$ iterations of $Q.\textsc{DeleteMin}()$, each of which takes $O(\log |V|)$ time.
    \item The updates $d(v) \leftarrow \min \{d(v), w(u, v)\}$ has amortized $O(1)$ cost with a Fibonacci heap, $O(|E|)$ total.
\end{itemize}
