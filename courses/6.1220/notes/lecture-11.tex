...
\textbf{Definition (Linear Programming).} We seek to optimize a linear objective function subject to linear equality and linear inequality constraints.  It will always look like:
...
\begin{itemize}
    \item Finding the max of a linear program $\mathcal{P}$ is the same as finding the min of $-\mathcal{P}$.  So we'll default max.
...
\textbf{Definition.} Given a set of constraints $A \vec{x} \leq \vec{b}$ of a linear program $\mathcal{P}$,
\begin{itemize}
    \item We say a \textbf{feasible solution} is some $\vec{\tilde{x}} \geq \vec{0}$ such that $A\vec{\tilde{x}} \leq \vec{b}$.
    ...
    \end{itemize}
    \item We say the \textbf{optimal value}, possibly $\infty$, is the supremum of $\vec{c} \cdot \vec{x}$ over $\textsc{Feasible}(\mathcal{P})$; if it is finite, it is achieved by some \textbf{optimal solution}.
    \begin{itemize}
...
There's this idea of duality that's also pretty obvious.  You want to \emph{minimize} the RHS of a nonnegative linear combination $\vec{y}$ of the constraints $A \vec{x} \leq \vec{b}$, constrained on the LHS of this linear combination being at least $\vec{c} \cdot \vec{x}$.
