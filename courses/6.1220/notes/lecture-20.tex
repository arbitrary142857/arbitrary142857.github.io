...
\begin{remark}
    Even if a process depends on more history than just the previous state, it is sometimes possible to interpret it as a Markov chain by extending the state space.
\end{remark}
...
\textbf{Definition (DG Terms).} Draw the \textbf{directed graph} (DG) of $W$: one vertex per state, with an edge $u \to v$ whenever $W(u, v) > 0$.  Two vertices $u, v$ are said to \textbf{communicate with each other} (denoted $u \leftrightsquigarrow v$) if there is a directed path from $u$ to $v$ and vice versa.  Equivalence classes under $\leftrightsquigarrow$ are called \textbf{strongly connected components}.  A component is \textbf{transient} if it has an outgoing edge, and \textbf{recurrent} otherwise.

\textbf{Definition (Period).} The \textbf{period} of a vertex $v$ is the GCD of the lengths of all directed walks from $v$ back to itself.  A vertex is \textbf{periodic} if its period is greater than $1$, and \textbf{aperiodic} otherwise.
...
\begin{itemize}
    \item The stationary distribution $\pi_*$ is unique if and only if the DG has a \emph{unique} recurrent component.
