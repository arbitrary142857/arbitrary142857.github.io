...
\begin{remark}
    Note that, in general, the bias, standard error, and MSE are all implicitly functions of $\theta$.
\end{remark}
...
\begin{theorem}{Consistency and MSE}
    If $\mathrm{MSE}(\hat{\theta}) \to 0$ as $n \to \infty$,
...
\begin{proof}
    ...
    \begin{quote}
        ...
        as $n \to \infty$. 
    \end{quote}
    Well, taking $X_n := (\hat{\theta}_n - \theta)^2$, we are given that
    $\mathbb{E}_{\theta}[X_n] \to 0$ as $n \to \infty$.  The finish is Markov's inequality:
    \[ \Pr_{\theta}(|\hat{\theta}_n - \theta| > \epsilon)
    = \Pr_{\theta}(X_n > \epsilon^2) \leq \frac{\mathbb{E}_{\theta}[X_n]}{\epsilon^2}
