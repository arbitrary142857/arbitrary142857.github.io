...
Recall that the MLE is the solution to an \emph{optimization} problem:
...
\begin{quote}
    ...
    The \href{https://en.wikipedia.org/wiki/Newton\%27s_method}{Newton-Raphson algorithm}
...
\begin{quote}
    ...
    \[ \theta_0 = \argmax_{\theta_0} \sum_{i = 1}^n (1 - Z_i) \log f_{\theta_0}(Y_i)
    ~~~ \text{ and } ~~~
    \theta_1 = \argmax_{\theta_1} \sum_{i = 1}^n Z_i \log g_{\theta_1}(Y_i). \]
...
\begin{quote}
    ...
    Still, it's a good enough estimate where the reasoning in \textbf{Idea \#1} still holds.
\end{quote}
...
\begin{theorem}{EM Algorithm Efficacy}
    The EM algorithm increases the log likelihood at every iteration; that is,
