...
\textbf{Example.} The response to a drug dosage $D$ is modeled by
$\mathcal{N}\left(E_0 + \frac{D \times E_{\text{max}}}{D + \mathrm{ED}_{50}},\ \sigma^2\right)$,
for some parameter $\theta = (\sigma^2, E_0, E_{\text{max}}, \mathrm{ED}_{50})$.
...
\begin{remark}
    ...
    \begin{itemize}
        ...
        the MLE is $\hat{\theta} := \frac{1}{n} \sum_{i = 1}^n X_i$.
        \item For the model $\{\mathrm{Unif}([0, \theta]) \mid \theta > 0\}$,
        the MLE is $\hat{\theta} := \max(X_1, \dots, X_n)$.
    \end{itemize}
\end{remark}

Note that $L_n(\theta)$ and $\ell_n(\theta)$ are
\emph{random} functions, since their behaviors are determined by \emph{random} samples
