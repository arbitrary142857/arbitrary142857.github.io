...
\begin{proof}
    ...
    \begin{quote}
        ...
        and $Q \in O_k(\mathbb{R})$.  But $\Sigma$ is positive semidefinite,
        so the $\lambda_i$ are all nonnegative,
...
Note that the above theorem only produces a valid PDF when $\Sigma$
is symmetric and positive definite, because all covariance matrices
...
\begin{solution}
    Compute $\mathbb{V}[X + 3Y]$ doesn't even depend on
...
\[ X_i = \mu_i + \sum_{j = 1}^k \lambda_j Z_j, ~ \text{ where } 
...
\begin{solution}
    ...
    and $Y = ZX$ where $Z \sim \mathrm{Unif}\{\pm 1\}$.)
