...
\begin{quote}
    ...
    \begin{enumerate}
        ...
        where the LHS of each equation is a known constant,
        and the RHS of each equation is a known function of
...
does not meet the Cramér-Rao Lower Bound.

\subsection{Motiviating Bootstrapping}
...
Then each of the $B$ batches gives its own value for the estimator $\hat{\theta}$.
...
to estimate the variance of $\hat{\theta}$ like so:
\[ \hat{\mathbb{V}[\hat{\theta}]}
= \frac{1}{B} \sum_{b = 1}^B \left(
    \hat{\theta}_b - \frac{1}{B} \sum_{c = 1}^B \hat{\theta}_c
\right)^2 \]
The issue, however, is that we might not have $Bn \gg n$ samples.
...
In other words, $\hat{\mathbb{P}}_n$ just returns
...
$\{X_1^*, \dots, X_m^*\}$ sampled (i.i.d.) from the empirical distribution
$\mathrm{Uniform}(\{X_1, \dots, X_n\})$.

\textbf{Example.} Given the $n = 3$ samples $\{5.2, 7.3, 6.8\}$,
one possible bootstrap sample of size $m = 4$
is $\{7.3, 7.3, 6.8, 7.3\}$.

Thus, we don't actually need to take all $Bn \gg n$ of those samples.
We can instead just take $B$ bootstrap samples of size $n$.
