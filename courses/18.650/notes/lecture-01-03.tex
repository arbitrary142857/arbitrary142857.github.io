\lectures{1,2,3}
\title{Random Variables and Convergence}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The sample mean is $\bar{X}_n$, whereas
        ...
        \end{itemize}
        \item The sample variance is $s_n^2$, whereas
...
\begin{theorem}{Sample Mean Stats}
    Suppose $X_1, X_2, \dots, X_n \sim \mathbb{P}$ are sampled i.i.d.,
...
\subsection{Convergence Definitions}
...
\textbf{Example.} Consider independent $X_n \sim \text{Ber}(\frac{1}{2})$
and $X \sim \text{Ber}(\frac{1}{2})$.  Then $X_n \rightsquigarrow X$ is true,
but $X_n \convprob X$ is not.

Expectedly, convergence in distribution is much less strict
...
\begin{proof}
    Chebyshev's, assuming finite variance; the point is that $\mathbb{V}[\bar{X}_n] \to 0$
...
\begin{remark}
    ...
    \end{quote}
    A counterexample is $X_n, X, Y \sim \mathcal{N}(0, 1)$ with $X \perp Y$,
    and $Y_n = -X_n$.
\end{remark}
...
\begin{theorem}{Continuous Mapping Theorem}
    ...
    Similarly, if $X_n \rightsquigarrow X$, then $g(X_n) \rightsquigarrow g(X)$.
\end{theorem}
...
\subsection{Applying Convergence Theorems}

Here's the set-up.

\begin{quote}
    \textbf{Set-Up.} Someone claims Harvard grads have a mean salary of below $120$K.
    To test this, you collect $X_1, X_2, \dots, X_{100}$ salaries (i.i.d.)
    and find that $\bar{X}_n = 121$K and $\hat{\sigma} = 0.3$K (in thousands).

    You'd like to say $\bar{X}_n \approx \mathcal{N}(120, 0.3^2/100)$;
...
Unbiasedness alone says nothing about large $n$, so we need one more fact.

\begin{theorem}{Sample Variance is Consistent}
    Assuming $\sigma^2 < \infty$, we have $s_n^2 \convprob \sigma^2$.
\end{theorem}

\begin{proof}
    The Law of Large Numbers doesn't apply to $s_n^2$ as written,
    since $s_n^2$ is not the mean of i.i.d. sampled random variables.
    Let's rewrite $s_n^2$ to fix that.
    \[
        s_n^2
        = \frac{1}{n - 1} \left[ \sum_{i = 1}^n (X_i - \bar{X}_n)^2 \right]
        = \frac{1}{n - 1} \left[
            \left(\sum_{i = 1}^n X_i^2\right) - n \cdot \bar{X}_n^2
        \right]
        = \frac{n}{n - 1} \left[
            \left(\frac{1}{n}\sum_{i = 1}^n X_i^2\right) - \bar{X}_n^2
        \right].
    \]
    Now the Law of Large Numbers does apply, twice:
    \begin{itemize}
        \item The $X_i^2$ are i.i.d. with finite mean $\sigma^2 + \mu^2$,
        so the LLN says
        $\frac{1}{n}\sum_{i = 1}^n X_i^2 \convprob \mathbb{E}[X_1^2]$.
        \item The LLN says $\bar{X}_n \convprob \mu$.
        Then the Continuous Mapping Theorem (with $g(x) = x^2$)
        gives $\bar{X}_n^2 \convprob \mu^2$.
    \end{itemize}
    Convergence of Sums then handles the bracketed part of $s_n^2$.
    \[ \left(\frac{1}{n}\sum_{i = 1}^n X_i^2\right) - \bar{X}_n^2
    \convprob \mathbb{E}[X_1^2] - \mu^2 = \sigma^2. \]
    As for the leftover $\frac{n}{n - 1}$ factor,
    note that $\frac{n}{n - 1} \convprob 1$,
    so convergence of products yields
    $s_n^2 \convprob 1 \cdot \sigma^2 = \sigma^2$,
    as desired. ~ $\blacksquare$
\end{proof}

With the above, we can justify $\bar{X}_n \approx \mathcal{N}(120, 0.3^2/100)$ as follows:
\begin{enumerate}
    \item By consistency of the sample variance, $s_n^2 \convprob \sigma^2$.
