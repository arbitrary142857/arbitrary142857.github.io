...
\begin{remark}
    ...
    \begin{itemize}
        ...
        it tends to be that $\Cov(X_i, X_j) < 0$.
    \end{itemize}
    And here are some algebraic properties of covariance.
    \begin{itemize}
        \item $\Cov(X, Y) = \Cov(Y, X)$.
        \item $\Cov(aX + b, cY + d) = ac \Cov(X, Y)$.
        \item $\Cov(A + B, C) = \Cov(A, C) + \Cov(B, C)$.
        \begin{itemize}
            \item However, $\Cov(AB, C) \neq \Cov(A, C) \cdot \Cov(B, C)$.
            This wouldn't even pass dimensional analysis!
        \end{itemize}
...
\textbf{Definition.} The \textbf{variance (covariance matrix)}
of a random vector $X \in \mathbb{R}^k$ is the $k \times k$ matrix of pairwise covariances.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item $\mathbb{V}[A^{\top}X + b] = A^{\top}\Sigma A$ is\dots the exact same derivation as earlier.
    \end{itemize}
...
\begin{remark}
    ...
    $\Lambda = \mathrm{diag}(\lambda_1, \dots, \lambda_k)$ (eigenvalues) and $Q$ is the orthonormal eigenbasis.

    Furthermore, $\Sigma$ is positive semidefinite: for any eigen-pair $(q_i, \lambda_i)$, we have:
    \[ \mathbb{V}[q_i^{\top}X] = q_i^{\top} \Sigma q_i
    = q_i^{\top} (\lambda_i q_i) = \lambda_i \cdot \| q_i \|^2. \]
    ...
    \emph{if and only if} $M$ is symmetric positive semidefinite.

    Furthermore, $\frac{1}{c^{\top}c}\Var(c^{\top}X)$ is maximized when $c$ is the eigenvector
    with greatest eigenvalue.  See 18.701 notes; this gives rise to Principal Component
    Analysis (PCA), where the eigenvectors are the principal components of $X$.
\end{remark}
