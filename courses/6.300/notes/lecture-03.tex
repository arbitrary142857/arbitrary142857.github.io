...
\subsection{Lecture: Complex Fourier Series}
...
\[ c_k = a_k + a_{(-k)} ~ \text{ and } ~ d_k = j\left(a_k - a_{(-k)}\right) ~ \text{ for } k \geq 1. ~~~ \parallel ~~~
a_k = \begin{cases}
c_0 & \text{ for } k = 0. \\
...
\begin{theorem}{Complex Fourier Coefficients}
    Given a signal $f(t)$ with period $T$
    and fundamental frequency $\omega_0 := 2\pi / T$, we have:
    \[ a_{\ell} = \dfrac{1}{T} \int_T f(t) e^{-j \omega_0 \ell t} \, \mathrm{d}t. \]
    Here, $\int_T$ denotes an integral over any interval of length $T$.
...
\begin{solution}
    Sparing you the computational details, you can verify:
    \[
    c_k' = \begin{cases}
...
As we saw in lecture, shifting a signal just multiplies
each of its complex Fourier coefficients by a unit-magnitude constant.
...
\begin{itemize}
    \item A signal $f(t)$ is real iff $\{a_k\}$ is Hermitian symmetric: $a_{-k} = \overline{a_k}$.
    \begin{itemize}
        \item A real $f(t)$ is symmetric iff every $a_k$ is purely real.
        \item A real $f(t)$ is antisymmetric iff every $a_k$ is purely imaginary.
    \end{itemize}
\end{itemize}
...
\begin{theorem}{Parseval's Theorem}
    ...
    we have $\frac{1}{T} \int_T |f(t)|^2 \, \mathrm{d}t = \sum_{k = -\infty}^{\infty} |a_k|^2$.
\end{theorem}

\begin{proof}
    Just use $z \overline{z} = |z|^2$ and the fact that
    $\{e^{jk \omega_0 t}\}_{k = -\infty}^{\infty}$ forms an orthonormal basis
    under $\langle g, h \rangle := \frac{1}{T} \int_T g(t) \overline{h(t)} \, \mathrm{d}t$. ~ $\blacksquare$
\end{proof}
