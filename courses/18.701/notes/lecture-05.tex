...
\begin{remark}
    All fields $F$ contain $\mathbb{Z}$; that is, it makes sense to refer to ``$5$'' or ``$-67$'' as elements of $F$.  This is because $1$ is an element of $F$, so when we refer to ``$5$'' in $F$, we're just referring to ``$1 + 1 + 1 + 1 + 1$''.
\end{remark}
...
\begin{proof}
    Since $0$ is the additive inverse, $y + 0 = y$.  Multiplying by $x$ on both sides yields $x \cdot y + x \cdot 0 = x \cdot y$.  Subtracting $x \cdot y$ from both sides yields $x \cdot 0 = 0$.
\end{proof}

\begin{theorem}{Zero Product Property}
    For any $x, y \in F$, if $xy = 0$, then either $x = 0$ or $y = 0$. ~ $\blacksquare$
\end{theorem}
...
\begin{proof}
    ...
    If $n$ is prime, then we claim all $a \in \mathbb{Z}/n\mathbb{Z}$ have a multiplicative inverse.  Here are two proofs:

    \underline{Proof \#1:} Consider the subgroup $\langle a \rangle$ of $\mathbb{Z}/n\mathbb{Z}$.  By Lagrange's Theorem, the order of this subgroup divides $n$.  But $n$ is prime, so the order of this subgroup must either be $1$ or $n$.
    
    In the former case, $\langle a \rangle = \{1\}$.  And in the latter case, $\langle a \rangle = \mathbb{Z}/n\mathbb{Z}$.  In either case, $\langle a \rangle$ contains $1$, meaning there is some integer $b \in \mathbb{Z}$ such that $ab = 1$, as desired. ~ $\square$

    \underline{Proof \#2:} Equivalently, it suffices show that for any $a \in \mathbb{Z}$ such that $\gcd(a, n) = 1$, there must exist $b, c \in \mathbb{Z}$ such that $ab + nc = 1$.  The key idea is to consider the group $a\mathbb{Z} + n\mathbb{Z}$.  It must be a subgroup of $\mathbb{Z}$, so it looks like $k\mathbb{Z}$ for some $k \in \mathbb{Z}$.  This $k$ must satisfy the property that $k \mid a$ and $k \mid n$.  But $\gcd(a, n) = 1$, so it must be that $k = 1$.
    
    In other words, $a\mathbb{Z} + n\mathbb{Z} = \mathbb{Z}$.  So there exists some $b, c \in \mathbb{Z}$ such that $ab + nc = 1$, exactly as desired. ~ $\square$
\end{proof}

Thus, the set $(\mathbb{Z}/n\mathbb{Z}, +, \cdot)$ is a field for primes $n$.  But be careful!  This does not hold when $n$ is a prime power, since (e.g.) $3 \in (\mathbb{Z}/9\mathbb{Z}, +, \cdot)$ has no multiplicative inverse.
...
\begin{itemize}
    \item It has two binary operations: vector addition $+: V \times V \to V$ and scalar multiplication $\cdot: F \times V \to V$.
...
\textbf{Definition (Matrices).} Consider vector spaces $F^m$ and $F^n$ over a field $F$.  Then a matrix $M \in F^{m \times n}$ is both (i) an rectangular array consisting of elements of $F$, and (ii) a linear map between vectors in $F^n$ to vectors in $F^m$.
\[ M = \begin{bmatrix} M_{11} & M_{12} & \cdots & M_{1n} \\ M_{21} & M_{22} & \cdots & M_{2n} \\ \vdots & \vdots & \ddots & \vdots \\ M_{m1} & M_{m2} & \cdots & M_{mn} \end{bmatrix} ~ \implies ~ M\begin{bmatrix} v_1 \\ v_2 \\ \vdots \\ v_n \end{bmatrix} = \begin{bmatrix} M_{11}v_1 + M_{12}v_2 + \cdots + M_{1n}v_n \\ M_{21} v_1 + M_{22} v_2 + \cdots + M_{2n} v_n \\ \vdots \\ A_{m1} v_1 + A_{m2}v_2 + \cdots + A_{mn} v_n \end{bmatrix} \]
...
\[ \text{For all } v_1, \dots, v_n \in S, \text{ we have } \left ( \sum_{i = 1}^n \lambda_i v_i = 0 \right) \iff \left( \lambda_i = 0 \text{ for all } i \right)  \]
...
\begin{proof}
    ...
    Subtracting these two linear combinations yields $0 = (\lambda_1 - \mu_1)v_1 + \dots + (\lambda_n - \mu_n)v_n$, which is a nontrivial linear combination of $S$ equal to zero.  So $S$ is not linearly independent, contradiction. ~ $\blacksquare$
\end{proof}
...
We want to be use the size of a basis of $V$ to talk about the dimension of $V$.  Here's a first step.
...
\begin{proof}
    ...
    \[ \text{For all } i \in \{1, \dots, s\}, ~ \text{ we have } \sum_{j = 1}^r A_{ij} v_j = w_i. \]
    ...
    Then if we perform row-reduction on $A$, the resulting matrix must have a column of zeroes.  This means that the there is some nonzero $x = \{x_1, \dots, x_s\}\in F^s$ such that $Ax = 0$.

    The finish is to violate the linear independence of $S_2$ by using $\{x_i\}$ as coefficients of a linear combination of $S_2$.
    \[ \sum_{j = 1}^s x_j w_j = \sum_{j = 1}^s x_j \left(\sum_{i = 1}^r A_{ij}v_i\right) = \sum_{i = 1}^r \left(\sum_{j = 1}^s A_{ij} x_j \right) v_i = \sum_{j = 1}^s (0)v_i = 0.  \]
