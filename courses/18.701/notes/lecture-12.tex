...
\begin{itemize}
    ...
    \item The \textbf{stabilizer} of $s$ is $\Stab_G(s) := \{g \in G: gs = s\} \subseteq G$.  \emph{(the subgroup of do-nothing actions)}
\end{itemize}
...
\begin{quote}
    ...
    Note that $\Stab_G(gH) = gHg^{-1}$ for all $g \in G$; in particular, $\Stab_G(H) = H$.  If $H \unlhd G$, then in fact $\Stab_G(gH) = H$ for every $g$.
\end{quote}
...
\begin{itemize}
    \item Since $G$ acts transitively on $V = \{\text{all $8$ vertices}\}$, and $|\Stab_G(v)| = 3$ for any vertex $v$, we have $|G| = 8 \times 3 = 24$.
    \item Since $G$ acts transitively on $E = \{\text{all $12$ edges}\}$, and $|\Stab_G(e)| = 2$ for any edge $e$, we have $|G| = 12 \times 2 = 24$.
    \item Since $G$ acts transitively on $F = \{\text{all $6$ faces}\}$, and $|\Stab_G(f)| = 4$ for any face $f$, we have $|G| = 6 \times 4 = 24$.
\end{itemize}
...
\textbf{Definition (Pole).} Let $S^2 := \{x \in \mathbb{R}^3: |x| = 1\}$ be the unit sphere.  Then every $g \in G \setminus \{\mathrm{id}\}$ has two \textbf{poles}, those being the two points of $S^2$ fixed by $g$.  Furthermore, we define:
...
Say the induced group action on the set of poles partitions it into $k$ orbits of sizes $n_1, \dots, n_k$.  We now count $|P|$ in two ways:
\begin{itemize}
    \item For each of the $n_i$ poles $p$ in the $i^{\text{th}}$ orbit, there are $\frac{N}{n_i} - 1$ nontrivial group actions that fix that pole; this is because of the Orbit-Stabilizer theorem.
...
\begin{itemize}
    ...
    \emph{Proof:} Recall that there are $\frac{N}{n_i} - 1 = r_i - 1$ nontrivial group actions that fix any pole in the $i^{\text{th}}$ orbit.  But every pole---by nature of being a pole---is fixed by at least $1$ nontrivial group action.  Thus, $r_i \geq 2$ for all $i$.
...
\begin{quote}
    \textbf{Rotation Symmetry Groups in $\mathbb{R}^3$.}
    \begin{itemize}
...
And that's all the finite rotation symmetry groups in $\mathbb{R}^3$.
