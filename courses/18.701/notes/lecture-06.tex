...
\textbf{Definition (Direct Sum).} If subspaces $W_1, \dots, W_k \subseteq V$ of a vector space $V$ are both spanning and linearly independent, then we say $V$ is isomorphic to the \textbf{direct sum} ($\oplus$) of these subspaces:
...
\begin{itemize}
    ...
    \item $T$ is an \textbf{isomorphism} if it is a bijective linear transformation.
\end{itemize}
...
\begin{theorem}{Finite-Dimensional Vector Spaces}
    ...
    So all vector spaces of the same dimension over $F$ are isomorphic.
\end{theorem}
...
\textbf{Physicists' Approach:} We can reduce any abstract $n$-dimensional vector space $V$ to the simpler vector space $F^n$, as long as we just pick a basis.  So the different ways we can pick a good basis should be our focus of study.
    ...
    \end{itemize}
\textbf{Mathematicians' Approach:} The same vector space $V$ looks very different under different choices of basis.  So our focus of study should be the qualities of vector spaces that are well-defined independent of choice of basis.
    \begin{itemize}
...
To fix this, let's take the physicists' approach: if we choose a basis for vector spaces $V$ and $W$, then we can reduce $V$ and $W$ to the isomorphic vector spaces $F^n$ and $F^m$, at which point matrix representations make sense.

\begin{theorem}{Matrix Representation of Linear Maps}
    Consider vector spaces $V$ and $W$ over a field $F$.  Select a basis $\{v_1, \dots, v_n\} \subseteq V$ for $V$ and a basis $\{w_1, \dots, w_m\} \subseteq W$ for $W$.  Then any linear transformation $T: V \to W$ is determined entirely by just the images of $T(v_1), \dots, T(v_n)$.  To be explicit,
    \begin{itemize}
        \item Express each $T(v_i) \in W$ as a linear combination of the basis $\{w_j\} \subseteq W$ with coefficients $(M_T)_{ji}$; that is, \vspace{-0.2cm}
        \[ \text{Define } (M_T)_{ji} \text{ to be the coefficients required to make } T(v_i) = \sum_{j = 1}^m (M_T)_{ji} w_j \text{ true.} \] \vspace{-0.7cm}
        \item Given these entries $\{(M_T)_{ji}\}$ alone, we can compute the image of \emph{any} vector $v = \sum_{i = 1}^n \lambda_i v_i \in V$ under $T$ to be: \vspace{-0.2cm}
...
Unfortunately, abstract vector spaces $V$ and $W$ aren't usually born with an inherent choice of basis.  How do the entries of our matrix $\{(M_T)_{ji}\} \in F^{m \times n}$ change when we change our choices of basis?

\begin{theorem}{Change of Basis}
    Suppose a linear transformation $T: V \to W$ has corresponding matrix representation $M_T$ when we choose bases $\{v_1, \dots, v_n\} \subseteq V$ and $\{w_1, \dots, w_m\} \subseteq W$.
    
    Suppose we instead adopt new bases $\{Av_1, \dots, Av_n\} \subseteq V$ and $\{Bw_1, \dots, Bw_m\} \subseteq W$, where $A$ and $B$ are bijective linear operators $A: V \to V$ and $B: W \to W$ that encode our change of basis.
    
    Then the matrix $M_T'$ determined by these new choices of bases looks like $M_T' = M_{B}^{-1}M_T M_A$.
\end{theorem}
...
\begin{proof}
    ...
    \[ (B^{-1}TA)v_i = \sum_{j = 1}^m (M_T')_{ji}w_j. \]
    Forget about what $M_T'$ meant to us to begin with: looking only at the equation above, we can read off that the coefficients $(M_T')_{ji}$ encode the image of each $v_i$ under the map $B^{-1}TA$ as a linear combination of $w_j$.
...
\begin{itemize}
    ...
    \item If we pick basis $\{v_1, \dots, v_n\} \subseteq V$ for $V$, then we can express $T$ using a matrix $M_T \in F^{n \times n}$.
...
\begin{remark}
    Recall that a subgroup is normal if it remains invariant under conjugation.  Equivalently, a subgroup $H$ of $GL_n(F)$ is normal if changing the basis of a matrix in $H$ yields another matrix in $H$.
\end{remark}
...
\begin{theorem}{Dimension Theorem}
    For a linear transformation $T: V \to W$ between finite-dimensional vector spaces $V$ and $W$, we have $\dim \im T + \dim \ker T = \dim V$.
\end{theorem}
...
\begin{remark}
    ...
    In fact, the Dimension Theorem is \emph{stronger} than $| \ker f | \cdot | \im f | = |G|$, since dimension is a stronger notion than cardinality: $\mathbb{R}^2$ and $\mathbb{R}^3$ have the same cardinality, but as vector spaces over $\mathbb{R}$, they have different dimensions!
\end{remark}
...
\begin{proof}
    ...
    \[ M_{T_0} = \begin{bmatrix}
        1 & \cdots & 0 & 0 & \cdots & 0 \\ \vdots & \ddots & \vdots & \vdots & \vdots & \vdots \\ 0 & \cdots & 1 & 0 & \cdots & 0  \\ 0 & \cdots & 0 & 0 & \cdots & 0 \\ \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\ 0 & \cdots & 0 & 0 & \cdots & 0 \end{bmatrix} \text{ with a } k\text{-dimensional } I_k \text{ in the top-left corner.} \]
    ...
    Now the key idea is the following: consider, more generally, a linear transformation $T: V \to W$ whose matrix representation $M_T$ is not as nice as $M_{T_0}$.  We claim that, under a suitable change of basis, the matrix $M_T$ can become $M_T' = M_{T_0}$, in which case we'd be done!

    Why must there exist a change of basis that transforms $M_T$ into $M_T' = M_{T_0}$?  Because performing change of basis on a matrix looks like performing row and column operations on $M_T$.  So we can procedurally perform row and column operations (changes of basis) to eventually reform $M_T$ into the form $M_{T_0}$. ~ $\blacksquare$
\end{proof}

The moral of the story?  Picking the right basis can yield very nice results.

\begin{remark}
    The above argument also implies that the rank of a matrix $M_T$ is the same as that of its transpose $M_T^{\top}$.
\end{remark}
