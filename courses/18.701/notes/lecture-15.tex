...
\textbf{Definition ($p$-Adic Valuation).} For primes $p$, let $\nu_p(k)$ denote the greatest nonnegative integer $\ell$ such that $p^{\ell} \mid k$.

\textbf{Definition ($p$-Subgroup, Sylow $p$-Subgroup).} For primes $p$, a subgroup $H$ of a group $G$ is a \textbf{$p$-subgroup} if $|H|$ is a power of $p$, and a \textbf{Sylow $p$-subgroup} if $|H| = p^{\nu_p(|G|)}$.
...
\begin{solution}
    Let $G$ be a group of order $10$.  Then by Sylow's Theorem \#4,
    \begin{itemize}
        \item There is exactly one Sylow $5$-subgroup $H = \langle x \rangle$ of $G$, with $|H| = 5$.
        \item There are either one or five Sylow $2$-subgroups $K$, with $|K| = 2$.  Let's say $K = \langle y \rangle$.
    \end{itemize}
...
\begin{proof}
    ...
    \[ G_n = \{ \langle x, y \rangle \mid x^q = y^p = \mathrm{id}, ~ yx = x^ny\} \text{ for some } n \in \{1,2, \dots, q - 1\}. \]
    ...
    \end{quote}
    So either $n = 1$ or $n \equiv n_0^k \pmod{q}$ for some $k \in \{1, 2, \dots, p - 1\}$.  The former yields $G \cong C_p \times C_q$, whereas the latter yields the semi-direct product $C_q \rtimes C_p$.  (For now, just trust that $C_q \rtimes C_p$ is correct.) ~ $\blacksquare$
\end{proof}

\begin{remark}
    Regarding Problem Set \#4\dots we also showed more strongly that the multiplicative group of any finite field is cyclic.  This implies, for example, that there is always a primitive root modulo any prime $p$.
\end{remark}
...
\begin{proof}
    ...
    \begin{quote}
        \textbf{Group Action.} View $G$ as a group action (via left multiplication) on the set $S$ of all subsets $T \subseteq G$ such that $|T| = p^{\ell}$.
    \end{quote}
    ...
    \begin{quote}
        ...
        Finally, $HU = U$ implies $U$ is a union of cosets of $H$, and so $|H|$ divides $|U|$.  But $U \in S$, so $|U| = p^{\ell}$ must hold.  Thus, $|H|$ divides $p^{\ell}$.
...
\begin{proof}
    ...
    \begin{quote}
        ...
        \begin{quote}
            \textbf{Group Action.} View $G$ as a group action on subsets of itself (via left multiplication).
        \end{quote}
...
\begin{proof}
    ...
    \begin{quote}
        ...
        \emph{Proof:} Consider the group $G$ acting on the set $X$ via conjugation.  Sylow \#2 says this is a transitive group action.  Then the Orbit-Stabilizer Theorem says $|G| = |X| \cdot |\Stab_G(H)|$ for any $H \in X$, so $|X|$ divides $|G|$. ~ $\square$
    \end{quote}
