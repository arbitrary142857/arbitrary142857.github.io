...
\textbf{Definition (Length / Norm / Distance).} For a vector $x \in \mathbb{R}^n$, the \textbf{length / norm} of $x$ is $|x| := \sqrt{\langle x, x \rangle}$.  Furthermore, the \textbf{distance} between $x, y \in \mathbb{R}^n$ is said to be $|x - y|$.
...
\begin{proof}
    The inequality $|x + \lambda y|^2 \geq 0$ holds for all $\lambda \in \mathbb{R}$.  Expanding this, we have:
...
\begin{theorem}{Orthogonal $\implies$ Independent}
    Any set of nonzero pairwise orthogonal vectors $\{a_1, \dots, a_k \} \subseteq \mathbb{R}^n$ must be linearly independent.  In particular, $n$ nonzero pairwise orthogonal vectors of $\mathbb{R}^n$ must form a basis.
\end{theorem}

\begin{proof}
    Given any linear dependence of $\{a_1, \dots, a_k\}$, we may write:
    \[ c_1a_1 + \dots + c_k a_k  = 0 ~ \implies ~ \langle a_i, c_1 a_1 + \dots + c_k a_k \rangle = 0 \text{ for all } i = 1, 2, \dots, k. \]
    But $\langle a_i, c_1 a_1 + \dots + c_k a_k \rangle = c_i |a_i|^2$ and $|a_i| \neq 0$, so $c_i = 0$ for all $i$. ~ $\blacksquare$
\end{proof}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item $(\#4) \iff (\#3)$.  Suppose the columns of $A$ form vectors $\{a_1, \dots, a_n\} \subseteq \mathbb{R}^n$.  Observe that:
        ...
        \item $(\# 5) \iff (\#3)$. If $A^{\top} A = I_n$, then $A^{\top}$ and $A$ are inverses.  So we also get $A A^{\top} = I_n$.  Now repeating the argument in $(\#3) \implies (\#4)$ with the equation $AA^{\top} = I_n$ implies the rows of $A$ are orthonormal.
    \end{itemize}
...
\textbf{Example.} By inspection, for $n = 2$, we can describe the elements of $O_2$ and $SO_2$ as rotations and reflections:
...
Note that the top entries of $A$ must be zero because the columns of $A$ must be orthonormal.  And the submatrix of $\heartsuit$s is in $SO_2(\mathbb{R})$.  So any transformation $A \in SO_3(\mathbb{R})$ may be understood as a planar rotation about an axis $v$.

\textbf{Example.} It turns out that elements of $SO_4(\mathbb{R})$ (in appropriate coordinates) look like the composition of rotations in two separate two-dimensional subspaces.
