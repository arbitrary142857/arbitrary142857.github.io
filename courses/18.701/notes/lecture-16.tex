...
\textbf{Definition (Symmetric, Semidefinite, Definite).} For a bilinear form $\langle \bullet, \bullet \rangle$ over an $\mathbb{R}$-vector space $V$,
\begin{itemize}
...
\begin{itemize}
    ...
    \dots decide on coefficients $A_{j, i}$ such that $Tv_i = \sum_{j} A_{j, i}w_j$.  Then $T$ is represented by the matrix $A$.
...
\begin{remark}
    Only if we're working in a field $F$ where $2 \neq 0$.  If $F = \mathbb{F}_2$, for example, then there is no symmetric $A$ that yields $x^{\top}Ax = x_1x_2$.
\end{remark}
...
\[ f(x) \approx \dfrac{1}{2} x^{\top} \!\underbrace{\left[\dfrac{\partial^2 f}{\partial x_i \partial x_j}(0)\right]}_{A} x  + \underbrace{\left[\nabla f(0)\right]}_{b} \!^{\top}x + \underbrace{f(0)}_{c}. \]
...
What if bilinear forms were not actually bilinear and were forced to live in $\mathbb{C}$?
...
\begin{remark}
    This implies $A: V \to V$ preserves the dot product $\langle x, y \rangle = x^{\top}y$ if and only if $A^{\top}A = I_n$, as expected. \\ A subgroup of matrices that preserve a more general symmetric bilinear form is called an \emph{indefinite orthogonal group}.
\end{remark}
...
\textbf{Definition (Unitary).} Take the Hermitian form to be $\langle x, y \rangle = x^{\dagger}y$.  A matrix $A \in \mathbb{C}^{n \times n}$ is \textbf{unitary} if $\langle x, y \rangle = \langle Ax, Ay \rangle$ for all $x, y \in \mathbb{C}^n$.  Equivalently, $A$ is unitary if $A^{\dagger}A = I$.
...
\begin{proof}
    ...
    For the Hermitian form, the symmetry implies $\langle e_i, e_j \rangle = \overline{\langle e_j, e_i \rangle}$, which reads $A_{i, j} = \overline{A_{j, i}}$.  So $A^{\dagger} = A$. ~ $\blacksquare$
\end{proof}
...
\begin{proof}
    Suppose $v$ has eigenvalue $\lambda$, and consider the Hermitian form $\langle x, y \rangle = x^{\dagger}Ay$.  Then $\langle v, v \rangle = \overline{\langle v, v \rangle}$, so $\langle v, v \rangle \in \mathbb{R}$.
