...
\subsection{Empirical Risk Minimization \dots}

Let's start simple: what's the \emph{stupidest} thing we can do?

Well, one idea is to use our training data $\{(x^{(n)}, y^{(n)})\}_{n = 1}^N$ to
...
\subsection{\dots Fails Miserably}
...
\begin{remark}
    This section is written with the assumption of knowledge from
    18.650 up to Lecture 12.
\end{remark}

Rather than naively approximating $p(x, y)$ using the empirical distribution $\hat{p}(x, y)$,
let's suppose $p$ belongs to a \emph{parametric model}
and estimate the parameter of $p$ via the \emph{MLE}.

\begin{problem}{MLE Example}
...
\begin{itemize}
    ...
    it is very easy and fast to computationally find the MLE.
