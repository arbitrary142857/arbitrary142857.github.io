...
\begin{solution}{Independent Set}
    ...
    \begin{quote}
        \textbf{Process.} If $v \in I$, then remove $v$ with probability $\frac{1}{2}$. \\
...
\[ \text{For all } (I, I'), \text{ we have } W(I \to I') = \frac{1}{2 |V|} \text{ or } W(I \to I') = 0. \]
...
\begin{itemize}
    ...
    \item Convergence: The Markov chain is aperiodic because there are some cycles $v \to v$ of length $1$. 
\end{itemize}
...
\end{itemize}
The acceptance factor, in a sense, adjusts our generic Markov chain $Q$ to account for the distribution $F$ desired.
\end{remark}
...
\begin{problem}{Graph Colorings}
    Given a graph $G$, sample a coloring of its vertices with $q > \max\{\deg v\} + 1$ colors, uniformly at random across all colorings.
\end{problem}
...
\begin{proof}
    ...
    \[ W(\chi, \chi') = W(\chi', \chi) = \begin{cases} 0 & \text{if $\chi$ and $\chi'$ more than one vertex apart} \\ \dfrac{1}{nq} & \text{if $\chi$ and $\chi'$ are exactly one vertex apart} \end{cases} \]
    ...
    \end{quote}
    So connectivity holds.  Aperiodicity does, too, as there are edges $v \to v$.  I'm convinced. ~ $\blacksquare$
\end{proof}

\begin{remark}
    This fails when $q = \max \{ \deg v \} + 1$ because the graph is disconnected.  (Take $G = K_4$ and $q = 4$.)
\end{remark}
