...
\begin{solution}{Open Addressing}
    ...
    \[ h: \mathcal{U} \times \{0, 1, \dots, m - 1\} \to \{0, 1, \dots, m-  1\} ~ \text{ such that } ~ k \mapsto (h(k, 0), h(k, 1), \dots, h(k, m - 1)). \]
...
\begin{solution}{Optimal-Space Solution}
    ...
    \begin{itemize}
        ...
        Using $O(N_{\ell})$ time and $O(N_{\ell}^2)$ space, give the $\ell^{\text{th}}$ bucket a sampled hash function $h_{2, \ell}: h_1^{-1}(\ell) \to \{0, 1, \dots, N_{\ell}^2 - 1\} \in \mathcal{H}_{2, \ell}$.
    \end{itemize}
...
\begin{proof}
    ...
    To prove this, note that by Markov's inequality, it suffices to to show $\underset{h \sim \mathcal{H}_1}{\mathbb{E}}\left [ \sum_{\ell = 0}^{n - 1} N_{\ell}^2 \right ] < 2n$.  This takes some clever work:
    \begin{align*}
