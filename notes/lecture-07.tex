...
\begin{solution}{Open Addressing}
    ...
    \[ h: \mathcal{U} \times \{0, 1, \dots, m - 1\} \to \{0, 1, \dots, m-  1\} ~ \text{ such that } ~ u \mapsto (h(u, 0), h(u, 1), \dots, h(u, m - 1)). \]
...
\begin{proof}
    ...
    The rigorous proof goes $\underset{h}{\mathbb{E}}[X] = \displaystyle\sum_{j = 1}^m j \cdot \underset{h}{\mathrm{Pr}}[X = j] = \displaystyle \sum_{j = 1}^m \underset{h}{\mathrm{Pr}}[X \geq j] \leq \displaystyle\sum_{j = 1}^m \alpha^{j - 1} = \frac{1}{1 - \alpha}$, hooray. ~ $\blacksquare$

    Even more formally, we need to justify $\underset{h}{\mathrm{Pr}}[X \geq j] \leq \alpha^{j - 1}$, so we should be writing something like:
    \[ \underset{h}{\mathrm{Pr}}[X \geq j] = \underset{h}{\mathrm{Pr}}[E_1 \wedge E_2 \wedge \dots \wedge E_{j - 1}] = \underset{h}{\mathrm{Pr}}[E_1] \times \underset{h}{\mathrm{Pr}}[E_2 \mid E_1] \times \dots \cdot \underset{h}{\mathrm{Pr}}[E_{j - 1} \mid E_1 \wedge E_2 \wedge \dots \wedge E_{j - 2}]. \]
    And the $\ell^{\text{th}}$ term in this product is upper-bounded by $\frac{n - \ell + 1}{m - 1 + 1} \leq \frac{n}{m} = \alpha$, as desired. ~ $\blacksquare$
\end{proof}
...
\begin{solution}{Probing Methods}
    Pick any two hash functions $h_1, h_2: \mathcal{U} \to \{0, 1, m - 1\}$.  We can use:
    \begin{itemize}
        \item \textbf{Linear Probing.} Take $h(k, p) := (h_1(k) + p) \%m$.
        \item \textbf{Double Hashing Probing.} Take $h(k, p) = (h_1(k) + p \cdot h_2(k)) \%m$.
    \end{itemize}
\end{solution}
...
\begin{problem}{Static Dictionaries \& Perfect Hashing}
    We are given $\{(k_1, v_1), \dots, (k_n, v_n)\}$ up front, at which point only $\textsc{Search}(k)$ may be called.  We would like the following:
...
\begin{proof}
    This works in expected $O(n)$ pre-processing time because $\underset{h \sim \mathcal{H}}{\mathrm{Pr}}[\text{no collisions}] \geq \frac{1}{2}$.
    
    The proof is by Union Bound---try it yourself!  (When does \emph{universality} of $\mathcal{H}$ get used?) ~ $\blacksquare$
\end{proof}
...
\begin{solution}{Optimal-Space Solution}
    Take a hash function $h_1 \in \mathcal{H}_1$ that goes $h_1: \mathcal{U} \to \{0, 1, \dots, n - 1\}$.
    \begin{itemize}
        \item Suppose $N_{\ell} := |h_1^{-1}(\ell)|$ for all $\ell \in \{0, 1, \dots, n - 1\}$.
        \item Use the suboptimal-space solution to fit $N_{\ell}$ key-value pairs in the $\ell^{\text{th}}$ bucket.
        
        Using $O(N_{\ell})$ time and $O(N_{\ell}^2)$ space, give the $\ell^{\text{th}}$ bucket a hash function $h_{2, \ell}: h_1^{-1}(\ell) \to \{0, 1, \dots, N_{\ell}^2 - 1\}$.
    \end{itemize}
    Our data structure is then a \emph{two-dimensional} array.
\end{solution}

\begin{proof}
    It's easy to see that this has $O(1)$ search time, and trivial to check that it has $O(n)$ pre-processing time.

    It remains to show that this uses $O(n)$ space.  Well\dots we might get unlucky, and it might not.  But\dots!
    \[ \textbf{Claim.} ~~~~~ \underset{h \sim \mathcal{H}_1}{\mathrm{Pr}} \left [ \left(\sum_{\ell = 0}^{n - 1} N_{\ell}^2\right) > 4n \right ] < \dfrac{1}{2}. \]
    By Markov's inequality, it suffices to to show $\underset{h \sim \mathcal{H}_1}{\mathbb{E}}\left [ \sum_{\ell = 0}^{n - 1} N_{\ell}^2 \right ] < 2n$.  Here's the smart idea:
    \begin{align*}
