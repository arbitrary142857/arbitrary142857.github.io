...
\begin{solution}{FFT}
    ...
    \begin{quote}
        \textbf{Claim (Inverse Discrete Fourier Transform).} Given the values $\{p(\omega^k)\}_{k = 1}^n$ for some degree $(n - 1)$ polynomial $p$, we can compute $p(x)$ in $O(n \log n)$ time.
