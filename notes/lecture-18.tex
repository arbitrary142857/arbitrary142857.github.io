...
\textbf{Definition (Sublinear).} An algorithm is \textbf{sublinear} if it is solved in less than $O(n)$ time---less time than it takes to even read the whole input!  (Naturally, these are approximation algorithms.)
...
\begin{remark}
    Solving this problem exactly requires $O(nd)$ time.  To distinguish the graphs $K_{n/2} \sqcup K_{n/2}$ and $K_{n/2, n/2}$, one must check $\frac{n}{2} \times \frac{n}{2} = O(n^2)$ edges, which is in fact greater than $O(nd)$.
\end{remark}
...
\end{quote}
    If we ``catch'' a vertex $v$ as being part of a connected component with less than $\frac{2}{\epsilon d}$, we declare \textsc{No}.
...
\begin{remark}
    ...
    The list $(2, 1, 4, 3, \dots, 2n, 2n - 1)$ is another useful counterexample.  (There's only $\frac{n}{2}$ inversions!)
\end{remark}
...
\begin{solution}{Sorted}
    ...
    \begin{quote}
        ...
        \emph{Proof:} Consider the binary search trees for $x_i$ and $x_j$.
    ...
    \end{quote}
    The point is that if we remove all elements that are not good, then the remaining list is sorted.  Thus, if a list is not $\epsilon$-close to sorted, it must have at least $\epsilon n$ vertices.
    
    So our probability of success is at least $1 - \left(1 - \frac{1}{\epsilon}\right)^{C/\epsilon} \geq 1 - e^{-C}$, yippee. ~ $\blacksquare$
\end{solution}
