...
\textbf{Notation.} We say that $\mathcal{P}$ denotes (optimization) problems, $P$ denotes instances, and $\textsc{Opt}(P)$ denotes the optimal value of a problem $P$.  We say that $\mathcal{A}$ denotes algorithms, and $\mathcal{A}(P)$ denotes the return value of $\mathcal{A}$ with input $P$.
...
\begin{proof}
    ...
    And the bound is easy: for any matching $M$, we have $|V^*| \geq |M|$, but $|V| = 2 \cdot |M|$. ~ $\blacksquare$
\end{proof}
...
\begin{proof}
     ...
     Notice $d_i \geq \frac{m_{i - 1}}{|S^*|}$, because edges in $G_{i - 1}$ map to to vertices in $S^*$ so that each vertex in $S^*$ gets at most $d_i$ edges.
     ...
     Thus, when $\ell \approx |S^*| \cdot \log |E|$ we have $m_{\ell} \leq 1$, so we have $|V| \leq |S^*| \cdot \log|E|$, done. ~ $\blacksquare$
\end{proof}
